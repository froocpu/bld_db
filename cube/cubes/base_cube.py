...
class BaseCube(object):
    ...
    def __init__(self, N, white_plastic=False):
        ...
        self.font_size = 12.0 * (self.N / 5.0)
    ...
    def base_move(self, f, l, d):
        ...
        if f == Notation.UP_FACE_CHAR:
            ...
            for d in ds:
                self._rotate(
                    [
                        (self.facedict[Notation.FRONT_FACE_CHAR], range(self.N), l2),
        ...
        if f == Notation.FRONT_FACE_CHAR:
            ...
            for d in ds:
                self._rotate(
                    [
                        (self.facedict[Notation.UP_FACE_CHAR], range(self.N), l),
                        (self.facedict[Notation.LEFT_FACE_CHAR], l2, range(self.N)),
                        (
                            self.facedict[Notation.DOWN_FACE_CHAR],
                            range(self.N)[::-1],
                            l2,
                        ),
                        (
                            self.facedict[Notation.RIGHT_FACE_CHAR],
                            l,
                            range(self.N)[::-1],
                        ),
                    ]
    ...
    def render_views(self, ax):
        ...
        this cubes is very brittle; it will not work for all viewpoints
        ...
        for viewpoint, shift in [
            ...
            for f, i in self.facedict.items():
                ...
                psc = 1.0 - 2.0 * self.sticker_thickness
                corners = [
                ...
                ax.text(
                    ...
                    va="center",
                    rotation=0,
                    fontsize=self.font_size / (-zorder),
                )
        return None

    def _stickerpolygon(self, xdir, ydir, zdir, csz, j, k):
        small = 0.5 * (1.0 - self.sticker_width)
        large = 1.0 - small
        return [
            zdir
            - xdir
