        ...
        for ind, cell in enumerate(raw_algs):
            ...
            try:
                ...
                successes.append({"index": ind,
                                  ...
                                  "parity_flag": (
                                      True if parity_calculation == 1 and flipped_edge_count == 0 else False),
                ...
                failures.append({"original": ind, "failure_code": 0, "failure_description": str(e)})
            except BadMultiplierException as e:
                failures.append({"original": ind, "failure_code": 1, "failure_description": str(e)})
            except InvalidMoveException as e:
                failures.append({"original": ind, "failure_code": 2, "failure_description": str(e)})
            except BadSeparatorException as e:
                failures.append({"original": ind, "failure_code": 3, "failure_description": str(e)})
            except UnclosedBracketsException as e:
                failures.append({"original": ind, "failure_code": 4, "failure_description": str(e)})
            except InvalidSequenceException as e:
                failures.append({"original": ind, "failure_code": 5, "failure_description": str(e)})
            except AlgorithmDoesNothingException as e:
                failures.append({"original": ind, "failure_code": 6, "failure_description": str(e)})
            except TooManyUnsolvedPiecesException as e:
                failures.append({"original": ind, "failure_code": 7, "failure_description": str(e)})
            except IllegalCharactersException as e:
                failures.append({"original": ind, "failure_code": 8, "failure_description": str(e)})
            except Exception:
                continue

        final_dataset.update({"data": {
            "0": {"values": {"0": {"cells": cells,
                               "successes": successes,
                               "failures": failures}}}}})
